...
class UserControlLer{
  ...
  Future<String> createUser(User user) async {
    return await usersCollection.add(user.toJson())
    .then((DocumentReference documentRef) => documentRef.id)
    .catchError((error) => throw Exception(error));
  }

  Future<User?> findUser(userId) async {
    Map<String, dynamic> userFromSnapShot = await usersCollection.doc(userId).get()
    .then((DocumentSnapshot doc) => doc.data() as Map<String, dynamic>)
    .catchError((error) => throw Exception(error));
    
    final List<Objective> objectives = [];

    userFromSnapShot['objectives'].forEach((objective) async => 
      objectives.add(
        Objective(
          name: objective['name'],
          phrases: await Stream.fromIterable(objective['phrases'])
        .map((phraseFromObjective) => Phrase(text: phraseFromObjective['text'])).toList()
        )
      )
    );
    ...
    return User(
      ...
      .map((phraseFromUser) => Phrase(text: phraseFromUser['text'])).toList());
  }
